...
class AlpacaExecutor:
    def __init__(self):
        ...
        self.mock_mode = os.getenv("MOCK_ALPACA", "true").lower() == "true"
    ...
    def submit_order(
        ...
        """
        Submits a buy/sell order to Alpaca.
        side: "buy" or "sell"
        ...
        if self.mock_mode:
            ...
            # Create a mock order structure with attribute access mimicking the Alpaca SDK Entity/Order
            class MockOrder:
            ...
            logger.info(f"[MOCK] Submitted {side.upper()} order for {qty} {symbol} (ID: {order_id})")
            return mock_order
        else:
            # Live Alpaca API submit_order call
    ...
    def get_order_status(self, order_id: str) -> Any:
        ...
        if self.mock_mode:
            ...
            # Transition mock order from 'new' to 'filled' on status check
            if mock_order.status == "new":
                mock_order.status = "filled"
                mock_order.filled_qty = mock_order.qty
                # Set dummy fill price if limit price exists, otherwise standard 100.00
            ...
            # Live Alpaca API get_order status check
