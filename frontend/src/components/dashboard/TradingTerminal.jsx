...
export default function TradingTerminal() {
  ...
  const handleClosePosition = (pos, currentPriceVal) => {
    navigate("/override", {
      state: {
        proposal: {
          symbol: pos.symbol,
          qty: pos.qty,
          entry_price: currentPriceVal || pos.current_price || 100.0,
          SL: currentPriceVal ? +(currentPriceVal * 1.02).toFixed(2) : +(pos.avg_entry_price * 1.02).toFixed(2),
          TP: currentPriceVal ? +(currentPriceVal * 0.95).toFixed(2) : +(pos.avg_entry_price * 0.95).toFixed(2),
          side: "SELL",
          signal: "SELL"
        }
      }
  ...
  return (
    <div className="flex flex-col gap-6 w-full px-1 sm:px-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card/10 border border-border/50 rounded-2xl p-4 md:px-6">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-foreground">Workspace Terminal</h2>
          <p className="text-xs text-muted-foreground">Monitor performance metrics, watch technical charts, and execute direct overrides.</p>
        </div>
        <button
          onClick={() => window.dispatchEvent(new Event("start-kerdostat-tour"))}
          className="flex h-9 items-center gap-1.5 rounded-xl border border-border bg-card/60 px-4 text-xs font-bold text-muted-foreground hover:border-primary/50 hover:text-primary transition duration-200"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6" data-testid="stats-summary-row">
        ...
        <Card className="rounded-2xl border border-border bg-card p-5 shadow-sm flex flex-col justify-between hover:border-primary/30 transition duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Buying Power</span>
            <div className="h-8 w-8 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 grid place-items-center">
              <Zap className="h-4 w-4" />
            </div>
          </div>
          <div className="mt-4 space-y-1">
            <h3 className="text-2xl font-bold text-foreground tracking-tight font-sans">
              ${(accountInfo.buying_power || 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </h3>
            <p className="text-[10px] text-muted-foreground">Available Margin Limit</p>
          </div>
        </Card>
        ...
        <Card className="rounded-2xl border border-border bg-card p-5 shadow-sm flex flex-col justify-between hover:border-primary/30 transition duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Cash Balance</span>
            <div className="h-8 w-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 grid place-items-center">
              <DollarSign className="h-4 w-4" />
            </div>
          </div>
          <div className="mt-4 space-y-1">
            <h3 className="text-2xl font-bold text-foreground tracking-tight font-sans">
              ${(accountInfo.cash || 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </h3>
            <p className="text-[10px] text-muted-foreground">Settled/Borrowed Funds</p>
          </div>
        </Card>

        {/* Card 4: Daily Performance P&L */}
        <PnLCard dailyPnl={accountInfo.daily_pnl || 0.0} winRate={68.5} tradeCount={12} />

        {/* Card 5: Active Exposure */}
        <Card className="rounded-2xl border border-border bg-card p-5 shadow-sm flex flex-col justify-between hover:border-primary/30 transition duration-200">
          <div className="flex items-center justify-between">
        ...
        <Card className="rounded-2xl border border-border bg-card p-5 shadow-sm flex flex-col justify-between hover:border-primary/30 transition duration-200" data-testid="safety-card">
          <div className="flex items-center justify-between">
            ...
            <div className={`h-8 w-8 rounded-lg grid place-items-center ${isBreached
          ...
          <div className="mt-4 space-y-1">
            <h3 className="text-2xl font-bold text-foreground tracking-tight flex items-center gap-1.5 font-sans">
              {isBreached ? (
                <>
      ...
      <div className="w-full">
        <Card className="flex flex-col rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-sm min-h-[380px] w-full" data-testid="chart-section-card">
                ...
                <div className="flex items-center gap-1.5 bg-secondary/20 p-0.5 rounded-lg border border-border/40">
                  {["AAPL", "NVDA", "TSLA", "MSFT"].map((sym) => (
                    <button
                      ...
                      type="button"
                      className={`px-2 py-0.5 text-[10px] font-bold rounded transition ${selectedSymbol === sym
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6">
        <Card className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-sm overflow-hidden" data-testid="positions-section-card">
              ...
              <Table className="w-full min-w-[500px] text-sm text-foreground">
                <TableHeader>
                  <TableRow className="border-b border-border hover:bg-transparent">
                    ...
                    <TableHead className="font-semibold text-muted-foreground text-center py-3">Action</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {positions.length > 0 ? (
                    positions.map((pos) => {
                      ...
                      return (
                        <TableRow
                          key={pos.symbol}
                          className={`border-b border-border/50 hover:bg-secondary/20 cursor-pointer ${selectedSymbol === pos.symbol ? "bg-secondary/15" : ""
                            }`}
                        ...
                        >
                          ...
                          <TableCell className="text-right py-4">
                            <Badge
                              className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${updatedPnl >= 0
                          ...
                          <TableCell className="text-center py-4">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleClosePosition(pos, currentP);
                              }}
                              className="px-2.5 py-1 text-[10px] font-bold rounded-lg bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 hover:bg-rose-500/20 transition duration-200"
                            >
                              Close Position
                            </button>
                          </TableCell>
        ...
        <Card className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-sm" data-testid="xai-feed-card">
          <CardHeader className="space-y-1 border-b border-border pb-4 p-0">
            <CardTitle className="text-lg font-bold text-foreground">
